...
namespace Tests
{
    ...
    public class MainClassTests
    {
        ...
        {
            var msgPack = new BigMessageClass(true);

            int size = msgPack.GetSize();
            size.TestIsGreater(0);

            using PooledBuffer buffer = Tools.PackMsgToBuffer(ref msgPack, size, BigMessageClass.Pack);
            byte[] array = Tools.PackMsgToArray(ref msgPack, size, BigMessageClass.Pack);

            using var memStr = new MemoryStream();
            Tools.PackMsgToStream(ref msgPack, memStr, size, BigMessageClass.Pack);
            memStr.Position = 0;

            var msgUnpack = new BigMessageClass();
            Tools.UnpackMsg(ref msgUnpack, memStr, BigMessageClass.Unpack);

            msgPack.TestEqual(msgUnpack);
            
            msgUnpack = new BigMessageClass();

            int index = 0;
            Tools.UnpackMsg(ref msgUnpack, array, ref index, BigMessageClass.Unpack);
